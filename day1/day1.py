...
You rush to see if you can help. Apparently, one of the Elves tripped and accidentally sent the sleigh keys flying
into the ocean!
...
and it even has an experimental antenna that should be able to track the keys if you can boost
its signal strength high enough; there's a little meter that indicates the antenna's signal strength
...
def three_increments_sliding_window(l):
    if len(l) < 3:
        return 0
    increments = 0
    previous_sum = sum(l[0:3])
...
if __name__ == "__main__":
    with open("day1_input1.txt") as f:
        input_list = list(map(int, map(lambda x: x.rstrip(), f.readlines())))
        print(number_of_increments(input_list))

    with open("day1_input2.txt") as f:
        input_list = list(map(int, map(lambda x: x.rstrip(), f.readlines())))
        print(three_increments_sliding_window(input_list))
